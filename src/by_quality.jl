...
function college_tuition(ds :: DataSettings; modelUnits :: Bool = true)
    target = :netPriceQual;
    rf = raw_net_price_qual_gpa(ds.tuitionYear);
...
function mean_work_hours_by_qual(ds :: DataSettings; modelUnits :: Bool = true)
...
function load_frac_enroll_by_qual(ds :: DataSettings)
    rf = raw_entry_qual_parental();
    dataV = CollegeStrat.read_row_totals(CollegeStrat.data_file(rf));
...
function cum_loans_qual_year(ds :: DataSettings;  modelUnits :: Bool = true)
    target = :cumLoans_qtM;

    T = ds.Tmax;
    outM = zeros(n_colleges(ds), T);
    for t = 1 : T
        outM[:, t] = cum_loans_qual(ds, t, modelUnits = modelUnits);
    end
...
function cum_loans_qual(ds :: DataSettings,  t :: Integer;
    ...
    rawFn = raw_cum_loans_qual_year(t);
...
function courses_tried_qual(ds :: DataSettings, t :: Integer; 
    ...
    rawFn = raw_credits_taken_qual_gpa(t);
...
function courses_tried_qual_year(ds :: DataSettings; modelUnits :: Bool = true)
    target = :coursesTried_qtM;

    T = ds.Tmax;
    outM = zeros(n_colleges(ds), T);
    for t = 1 : T
        outM[:, t] = courses_tried_qual(ds, t, modelUnits = modelUnits);
    end
...
function cdf_gpa_by_qual(percentileV :: AbstractVector, qualityV :: AbstractVector)
    df = read_cdf_gpa_by_qual();
    pctColV = df[!, :pctile];
